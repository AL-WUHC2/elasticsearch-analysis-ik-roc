...
class DictSegment implements Comparable<DictSegment> {
     ...
     * @param charArray
     */
    boolean fillSegment(char[] charArray) {
        return this.fillSegment(charArray, 0, charArray.length, 1);
    }
     ...
     * @param charArray
     */
    boolean disableSegment(char[] charArray) {
        return this.fillSegment(charArray, 0, charArray.length, 0);
    }
     ...
     * @param enabled
     */
    private synchronized boolean fillSegment(char[] charArray, int begin, int length, int enabled) {
        //获取字典表中的汉字对象
        ...
        if (ds == null && enabled == 0) return false;

        //处理keyChar对应的segment
        if (length > 1) {
            //词元还没有完全加入词典树
            return ds.fillSegment(charArray, begin + 1, length - 1, enabled);
        } else if (length == 1) {
            //已经是词元的最后一个char,设置当前节点状态为enabled，
            //enabled=1表明一个完整的词，enabled=0表示从词典中屏蔽当前词
            boolean change = ds.nodeState != enabled;
